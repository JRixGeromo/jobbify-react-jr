...
export default function EditServicePage() {
  ...
  useEffect(() => {
    if (service) {
      setFormData({
        ...
        subscriptionFrequency: service.sub_frequency,
        subscriptionDetails: service.sub_details,
      });

      if (service.tags) {
        setTags(JSON.parse(service.tags));
      }
      setDetailedDescription(service.detailed_desc);
    }
  }, [service]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        const { data } = await updateService({ variables: { id, input } });
      } catch (error) {
        console.error('Error creating jobs:', error);
      }

      toast.success('Service updated successfully!', {
        description: `has been updated to your services.`,
        action: {
